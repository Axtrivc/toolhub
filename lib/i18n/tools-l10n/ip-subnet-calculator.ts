...
export const ipSubnetCalculatorL10n: ToolL10n = {
  zh: {
    ui: {
      ...
      'copySummary': '复制摘要',
      'errEnterIp': '请输入带前缀的 IPv4 地址,例如 192.168.1.10/24',
      'errInvalidIp': '无效的 IPv4 地址 "{ip}" —— 应为 4 个 0–255 的八位组',
      'errInvalidMask': '无效的子网掩码 "{mask}"',
      'errInvalidPrefix': '无效的前缀 "/{prefix}" —— 请使用 0–32',
      'errMissingPrefix': '缺少前缀长度 —— 请在 IP 后加上 /24(或空格 + 掩码)',
      'errNonContiguousMask': '掩码 "{mask}" 不连续(必须是高位连续的 1 后跟 0)',
      'errPrefixRange': '前缀 /{prefix} 超出范围(0–32)',
      'errTwoParts': '请使用 192.168.1.10/24 或 192.168.1.10 255.255.255.0 这样的格式',
  ...
  es: {
    ui: {
      ...
      'errEnterIp': 'Introduce una dirección IPv4 con prefijo, p. ej. 192.168.1.10/24',
      'errInvalidIp': 'Dirección IPv4 no válida "{ip}" — se esperan cuatro octetos de 0–255',
      'errInvalidMask': 'Máscara de subred no válida "{mask}"',
      'errInvalidPrefix': 'Prefijo no válido "/{prefix}" — usa 0–32',
      'errMissingPrefix': 'Falta la longitud del prefijo — añade /24 (o un espacio + máscara) después de la IP',
      'errNonContiguousMask': 'La máscara "{mask}" no es contigua (los bits deben ser unos seguidos de ceros)',
      'errPrefixRange': 'El prefijo /{prefix} está fuera de rango (0–32)',
      'errTwoParts': 'Usa formatos como 192.168.1.10/24 o 192.168.1.10 255.255.255.0',
  ...
  de: {
    ui: {
      ...
      'errEnterIp': 'Gib eine IPv4-Adresse mit Präfix ein, z. B. 192.168.1.10/24',
      'errInvalidIp': 'Ungültige IPv4-Adresse "{ip}" — erwartet werden vier Oktette von 0–255',
      'errInvalidMask': 'Ungültige Subnetzmaske "{mask}"',
      'errInvalidPrefix': 'Ungültiges Präfix "/{prefix}" — verwende 0–32',
      'errMissingPrefix': 'Präfixlänge fehlt — hänge /24 (oder Leerzeichen + Maske) an die IP an',
      'errNonContiguousMask': 'Maske "{mask}" ist nicht kontinuierlich (Bits müssen 1-en gefolgt von 0-en sein)',
      'errPrefixRange': 'Präfix /{prefix} liegt außerhalb des Bereichs (0–32)',
      'errTwoParts': 'Verwende Formate wie 192.168.1.10/24 oder 192.168.1.10 255.255.255.0',
