...
export function makeUnitConverter(config: {
  ...
  return function UnitConverter() {
    ...
    const fromLabel = config.units[from]?.label ?? from
    const toLabel = config.units[to]?.label ?? to
    ...
    const summary = useMemo(() => {
      ...
      return [
        'Conversion Summary',
        `  Value: ${value} ${fromLabel}`,
        `  Result: ${fmtNum(result, digits)} ${toLabel}`,
        `  Formula: ${fmtNum(toNum(value))} ${fromLabel} = ${fmtNum(result, digits)} ${toLabel}`,
      ].join('\n')
    }, [value, from, to, result, digits, fromLabel, toLabel])

    // 导出内容(纯文本,人类可读;转换器输出简单,不必用 CSV)
    const downloadContent = useMemo(() => {
      if (!isFinite(result)) return summary
      return `${fromLabel} to ${toLabel} conversion\n\n${fmtNum(toNum(value))} ${fromLabel} = ${fmtNum(result, digits)} ${toLabel}\n\nGenerated by ToolHub`
    }, [summary, value, from, to, result, digits, fromLabel, toLabel])
