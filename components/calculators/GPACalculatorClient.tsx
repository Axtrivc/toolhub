...
export function GPACalculatorClient() {
  ...
  // 结果摘要(纯文本):供 Copy/Download 使用,标签复用既有 bundle key
  const gpaSummary = [
    `${L('yourGpa', 'Your GPA')}: ${fmtNum(result.gpa, 2)} (${L('on4Scale', 'On 4.0 scale')})`,
    `${L('totalCredits', 'Total credits')}: ${fmtNum(result.totalCredits, 1)}`,
    `${L('gradePoints', 'Grade points')}: ${fmtNum(result.totalPoints, 1)}`,
  ].join('\n')
  ...
  return (
    <div className="space-y-6">
      ...
      <ResultActions
        summary={gpaSummary}
        filename="gpa-calculator-result.txt"
        downloadContent={`${gpaSummary}\n\n${tuiUc('generatedBy', locale, 'Generated by ToolHub')}`}
        disabled={result.totalCredits <= 0}
      />
